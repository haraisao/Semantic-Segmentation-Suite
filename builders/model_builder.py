...
def build_model(model_name, net_input, num_classes, crop_width, crop_height,
                frontend="ResNet101", is_training=True, **kwargs):
	...
	if model_name in ["FC-DenseNet56", "FC-DenseNet67", "FC-DenseNet103"]:
	...
	elif model_name in [ "FRRN-A" , "FRRN-B"]:
	    network = build_frrn(net_input,
                        preset_model = model_name,
                        num_classes=num_classes, **kwargs)

	elif model_name in ["Encoder-Decoder", "Encoder-Decoder-Skip"]:
	    network = build_encoder_decoder(net_input,
                            preset_model = model_name,
                            num_classes=num_classes, **kwargs)

	elif model_name in ["MobileUNet", "MobileUNet-Skip"]:
	    ...
	    network = build_adaptnet(net_input, num_classes=num_classes, **kwargs)

	elif model_name == "custom":
	    network = build_custom(net_input, num_classes, **kwargs)
